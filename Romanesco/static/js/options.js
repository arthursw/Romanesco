  ...
  define(['utils', 'tinycolor', 'gui', 'colorpickersliders', 'jquery', 'paper'], function(utils, tinycolor, GUI) {
    ...
    Controller = (function() {
      ...
      function Controller(name, parameter, folder) {
        ...
        this.initialize(this.parameter.value || this.parameter["default"]);
        return;
      }

      Controller.prototype.initialize = function(value) {
        var controllerBox, firstOptionalParameter, object, _base;
        if ((_base = this.parameter).onChange == null) {
          _base.onChange = this.onChange;
        }
        object = {};
        object[this.name] = value;
        firstOptionalParameter = this.parameter.min != null ? this.parameter.min : this.parameter.values;
        controllerBox = this.folder.datFolder.add(object, this.name, firstOptionalParameter, this.parameter.max).name(this.parameter.label).onChange(this.parameter.onChange).onFinishChange(this.parameter.onFinishChange);
      ...
      onChange = function(value) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          item = _ref[_i];
          if (typeof ((_ref1 = item.data) != null ? _ref1[name] : void 0) !== 'undefined') {
      ...
      Controller.prototype.addItems = function(items) {
        var controlled;
        this.items = this.items.concat(items);
        controlled = this.items.first() || g.selectedTool;
        if (controlled.parameterControllers == null) {
          controlled.parameterControllers = {};
        }
        controlled.parameterControllers[this.name] = this;
      };
    ...
    ColorController = (function() {
      function ColorController(name, parameter, folder) {
        ...
        this.parameter = parameter;
        this.folder = folder;
        this.colorPickerChanged = __bind(this.colorPickerChanged, this);
        this.initializeColorPicker = __bind(this.initializeColorPicker, this);
        this.gradientCheckboxChanged = __bind(this.gradientCheckboxChanged, this);
      ...
      ColorController.prototype.initialize = function(value) {
        var controllerBox, object;
        object = {};
        object[this.name] = value;
        controllerBox = this.folder.datFolder.add(object, this.name).name(this.parameter.label).onChange(this.parameter.onChange).onFinishChange(this.parameter.onFinishChange);
        this.datController = this.folder.datFolder.__controllers.last();
        ...
        this.enableCheckboxJ[0].checked = value || parameter.defaultCheck;
        if (value.gradient != null) {
          this.gradient = true;
          value = value.gradient.stops[0].color.toCSS();
        }
        this.colorInputJ.ColorPickerSliders({
        ...
        this.colorInputJ.on('shown.bs.popover', this.initializeColorPicker);
      ...
      ColorController.prototype.getValue = function() {
        if (this.enableCheckboxJ[0].checked) {
          return colorInputJ.val();
        } else {
          return null;
        }
      };

      ColorController.prototype.setValue = function(value) {
        ColorController.__super__.setValue.call(this, value);
        this.enableCheckboxJ[0].checked = value != null;
        this.colorInputJ.trigger("colorpickersliders.updateColor", value);
      };

      ColorController.prototype.enableCheckboxChanged = function(event) {
        this.parameter.onChange(this.getValue());
      };

      ColorController.prototype.gradientCheckboxChanged = function(event) {
        this.gradient = event.target.checked;
        if (this.gradient) {
          g.tools['Gradient'].select(this);
        } else {
          g.tools['Select'].select();
        }
      };

      ColorController.prototype.initializeColorPicker = function(event) {
        var checkboxJ, containerJ;
        ...
        checkboxJ.find('input').click(this.gradientCheckboxChanged);
        if (this.gradient) {
      ...
      ColorController.prototype.colorPickerChanged = function(container, color) {
        color = color.tiny.toRgbString();
        if (this.gradient) {
          g.tools['Gradient'].colorChange(color, this);
        } else {
          this.parameter.onChange(color);
        }
    ...
    ControllerManager = (function() {
      ...
      ControllerManager.prototype.resetControllers = function() {
        var controller, folder, folderName, name, _ref, _ref1;
        _ref = this.folders;
        for (folderName in _ref) {
          folder = _ref[folderName];
      ...
      ControllerManager.prototype.updateControllers = function(tools, resetValues) {
        var controller, folder, folderName, folderParameters, name, parameter, toolType, _i, _len, _ref;
        if (resetValues == null) {
          resetValues = false;
        }
        this.resetControllers();
        for (toolType = _i = 0, _len = tools.length; _i < _len; toolType = ++_i) {
          name = tools[toolType];
          _ref = toolTypes.parameters;
          for (folderName in _ref) {
            ...
            for (name in folderParameters) {
              ...
              parameter.value = this.initializeValue(name, parameter, toolType.items.first());
              if (controller != null) {
                if (resetValues) {
                  controller.setValue(parameter.value);
                }
              } else {
                if (controller == null) {
              ...
              if (this.parameter.addController) {
                controller.addItems(toolType.items);
              }
      ...
      ControllerManager.prototype.setSelectedTool = function(tool) {
        ...
        tools = {};
        tools[tool.constructor.name] = {
          parameter: tool.constructor.parameters(),
          items: []
        };
